...
def generate_svg(model, tokenizer, prompt: str, max_new_tokens: int = 1024, temperature: float = 0.3) -> str:
    ...
    生成参数选择理由（优化后）：
    - temperature=0.3：适度低温，保留一定随机性鼓励多样化输出，避免陷入
      「单色重复圆」安全模板（temp=0.2 时模型过度偏好已学模板）。
    - repetition_penalty=1.1：适度惩罚重复，抑制退化循环，同时不破坏 SVG
      合法的结构重复（如多个属性）。
    - 不用 no_repeat_ngram_size：会禁掉 SVG 必需的 3-gram 重复。
    """
    messages = [
        {
            "role": "system",
            "content": (
                ...
                "(<path>, <circle>, <ellipse>, <rect>, <polygon>, <line>, <g>). "
...
def evaluate_model(model, tokenizer, samples, label, max_new_tokens=1024, temperature=0.3):
    """对每个 prompt 生成一次 SVG 并打分（单次采样，诚实反映模型能力）。

    优化说明：移除了原先的多采样取最佳（num_samples=2）和退化 retry 机制。
    原因：多采样取最佳是一种统计偏向，系统性抬高 fine_tuned 分数而对
    baseline（全空 SVG）无影响，人为放大 delta。单次采样让基座和微调
    在同一条件下评测，分数更可信。退化输出由 reward 函数的 anti_template
    和 non_degenerate 维度自然惩罚，无需在评测侧额外丢弃。
    """
    results = []
